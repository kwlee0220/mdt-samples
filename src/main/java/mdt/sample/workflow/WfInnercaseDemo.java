...
public class WfInnercaseDemo {
	public static final void main(String... args) throws Exception {
		...
		wfDesc.setId("innercase_demo_workflow");
		...
		descriptor = inspectSurfaceThickness(manager, "inspect_thickness");
		wfDesc.addTaskDescriptor(descriptor);

		// 두께 불량 탐지 결과를 DefectList에 누적.
		descriptor = updateDefectList(manager, "update_defect_list");
		descriptor.addDependency("inspect_thickness");
		wfDesc.addTaskDescriptor(descriptor);

		// DefectList를 기반으로 공정 시뮬레이션을 수행하여 평균 사이클 타임을 계산.
		descriptor = simulateProcess(manager, "simulate_process");
		descriptor.addDependency("update_defect_list");
		wfDesc.addTaskDescriptor(descriptor);
		
		// Innercase를 구성하는 Heater, Former, Trimmer의 사이클 타임을 활용하여 공정 최적화를 실행.
		descriptor = optimizeProcess(manager, "optimize_process");
		descriptor.addDependency("simulate_process");
		wfDesc.addTaskDescriptor(descriptor);
...
//		System.out.println(MDTModelSerDe.toJsonString(wfDesc));
	...
	private static TaskDescriptor updateDefectList(MDTInstanceManager manager, String id) {
		...
		descriptor.setType(AASOperationTask.class.getName());
		descriptor.setId(id);
		descriptor.addInputArgumentSpec("Defect", ArgumentSpec.taskOutput("inspect_thickness", "Defect"));
